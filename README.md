...
To add other processes to run automaticly, add a file ending with `.conf` in `/etc/supervisor/conf.d/` with a layout like this (`/etc/supervisor/conf.d/myprogram.conf`)
```ini
[program:myprogram]
...
2017-09-18 11:34:06,763 CRIT Set uid to user 0
...
This is the commands and environment variables  used inside nimmis based containers for extra functionality.
...
```
docker exec -ti <docker ID> set_tz Europe/Vienna
```
...
### TAGs
This image contains following versions of Ubuntu (schast/docker-ubuntu:<tag>):
